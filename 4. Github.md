...
before trying to push our code we have to get an access tokent from gihub. [**Profile--> Settings--> Developer Settings--> Personal access tokens**] Use **Fine-grained tokens**, write a name, select **only select repositories** and chose the repo, then click **add permissions**, select **contents**, change the access to **Read and write**, finally click generate token. copy the token code.
