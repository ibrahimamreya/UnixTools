...
## Link/Unlink Local To Remote (HTTPS)
...
```bash
git remote add origin  https://github.com/ibrahimamreya/git-tutorial.git
```
...
before trying to push our code we have to get an access tokent from gihub. [**Profile--> Settings--> Developer Settings--> Personal access tokens**] Use **Fine-grained tokens**, write a name, select **only select repositories** and choose the repo, then click **add permissions**, select **contents**, change the access to **Read and write**, finally click generate token. copy the token code.

Now push your code and you will be prompted for a password just paste the token.
...
If the SSH test fails add a file named `config` inside the `~/.ssh` directory with the following contents:
